...
public abstract class AbstractEntity {
	...
	protected abstract ArrayList<XMLElement> getChildXMLElements() throws SecurityException, IllegalArgumentException, NoSuchMethodException, ClassNotFoundException, IllegalAccessException, InvocationTargetException;
	...
	protected void mapResultSet(ResultSet rs) throws SQLException, ReflectionException {
		...
		while (it.hasNext()) {
			...
			try {
				...
				System.err.println(String.format("Reflection error: %s, %s, %s", className, columnName, setterName));
	...
	public XMLElement getXML(boolean suppressAttributes) throws SecurityException, IllegalArgumentException, NoSuchMethodException, ClassNotFoundException, IllegalAccessException, InvocationTargetException {
		ArrayList<ColumnMapping> cols = getColumnMappings();
		XMLElement nodeElement = new XMLElement();
		
		nodeElement.setName(getXMLTag());

		String typeAttr = getTypeAttribute();
		if(typeAttr!=null && !suppressAttributes)
			nodeElement.addAttribute("type", typeAttr);
		
		// set each field
		for(ColumnMapping col : cols) 
			nodeElement.appendChild(getXMLElements(col));		
			
		ArrayList<XMLElement> childElements = getChildXMLElements();
	...
	protected XMLElement getXMLElements(ColumnMapping col) throws SecurityException, NoSuchMethodException, ClassNotFoundException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
		...
		childElement.setName(col.getAttributeName());
